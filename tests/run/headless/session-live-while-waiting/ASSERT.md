...
- Headless parent still running after session-id line is read.
...
func Assert(t *testing.T, req *Request, resp *Response, err error) {
	...
	if !resp.RegistryExists {
		t.Fatalf("registry should exist while headless parent waits")
	}
	assertRegistryHasSession(t, req.TTYWatchHome, resp.SessionID)
	if !resp.SessionRunning {
		t.Fatalf("session %s should be reachable while headless waits", resp.SessionID)
	}
